    ...
    >
      <SwipeItem v-for="item in bannerList" :key="item.id">
        <div
          ...
          @click="onBannerClick(item, 'click_playearn_banner')"
        ></div>
      </SwipeItem>
    </Swipe>
  </div>
...
export default class VPlayEarnBanner extends Mixins(
  BindEventMixinDefault,
  createOrGetVBannerListMixin('GAME_PLAYER_CENTER')
) {
  useInited() {
  ...
  onExposeChange() {
    ...
    const bannerItem = bannerList[this.curIdx];
    if (bannerItem) {
      this.tryTraceBannerExpose(bannerItem, 'playearn_banner_exposure', '1');
    }
  }
}
</script>
