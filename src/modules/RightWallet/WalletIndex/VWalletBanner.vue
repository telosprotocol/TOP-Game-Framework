    ...
    >
      <SwipeItem v-for="item in bannerList" :key="item.id">
        <div
          ...
          @click="onBannerClick(item, 'click_wallet_banner')"
        ></div>
      </SwipeItem>
    </Swipe>
  </div>
...
export default class VWalletBanner extends Mixins(
  BindEventMixinDefault,
  createOrGetVBannerListMixin('WALLET_FOOTER')
) {
  useInited() {
  ...
  onExposeChange() {
    ...
    const bannerItem = bannerList[this.curIdx];
    if (bannerItem) {
      this.tryTraceBannerExpose(bannerItem, 'wallet_banner_exposure');
    }
  }
}
</script>
