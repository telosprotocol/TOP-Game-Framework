<template>
  <portal to="modal">
    <GameWeekCardDlg v-model="isDlgShow" @close="onDlgClose"></GameWeekCardDlg>
  </portal>
</template>
...
import GameWeekCardDlg from './GameWeekCardDlg.vue';
@Component({
  components: { GameWeekCardDlg },
})
export default class GameWeekCardEntryLogic extends Vue {
  isDlgShow = false;
  ...
  onDlgClose(needUpdate: boolean) {
    if (needUpdate) {
      ...
      this.$emit('updateActivity', {
        type: 'WEEK_RECHARGE' as IActivityType,
      });
    }
  }
