          ...
          <div class="px-4">
            ...
            <div>
              ...
              >
                {{
                  hasWithdraw
                    ? $t('VRM.withdrawSmallX', [smallWithdrawTxt || '300'])
                    : $t('VRM.ToPlayGame')
                }}
              </button>
              <button
...
import { ROUTENAME_INAPP_V_REFER_MONEY } from '@/constants/localRoutePath';
@Component({
  components: {},
})
export default class VReferMoneyRewardClaimOkDlg extends mixins(PopupMixin) {
  @Prop({
    type: Number,
  })
  coinNum?: number;
  ...
  onOkClicked() {
    ...
    if (this.hasWithdraw) {
      openAppH5LinkPreferLocal(
        getGoldWithdrawUrl(ROUTENAME_INAPP_V_REFER_MONEY),
        {}
      );
    } else {
