...
ConcurrentStorage.prototype.get = function (
  key,
  lockTimeout,
  checkTime,
  callback
...
ConcurrentStorage.prototype.set = function (
  key,
  val,
  lockTimeout,
  checkTime,
...
function getDomBySelector(selector) {
  ...
  function getDom(selector, parent) {
    ...
    if (selector.indexOf('#') === 0) {
      ...
      if (
        !(isElement(parent) && parent.children && parent.children.length > 0)
      ) {
        return null;
      }
...
function sdLog() {
  if (
    ...
    if (
      ...
      (sdPara.show_log === true ||
        sdPara.show_log === 'string' ||
        sdPara.show_log === false)
    ) {
...
function decryptIfNeeded(cross) {
  if (
    isString(cross) &&
    (cross.indexOf(flag) === 0 || cross.indexOf(flag_dfm) === 0)
  ) {
    cross = decrypt(cross);
  }
  return cross;
...
function addEvent$1(target, eventName, evenHandler) {
  var useCapture =
    isObject(sdPara.heatmap) && sdPara.heatmap.useCapture ? true : false;
  if (
...
function isNewUser() {
  ...
  if (cookie$1.isSupport()) {
    ...
    if (memory.get(memory.getNewUserFlagMemoryKey(prefix)) !== null)
      return true;
    return false;
  }
}
    ...
    if (sd.para.preset_properties.search_keyword_baidu) {
      if (isReferralTraffic(document.referrer)) {
        if (isBaiduTraffic() && !(isObject(baiduKey) && baiduKey.active)) {
          ...
          latestObj['$search_keyword_id_hash'] = hashCode53(
...
var store = {
  ...
  init: function () {
    function compatibleWith3(state) {
      ...
      if (
        state.identities &&
        isObject(state.identities) &&
        !isEmptyObject(state.identities)
      ) {
        if (
...
function check(p, onComplete) {
  for (var i in p) {
    if (
      ...
      !checkOption.check(i, p[i], onComplete)
    ) {
      return false;
    }
  }
...
var heatmap = {
  ...
  isUnlimitedTag: function (el) {
    ...
    return (
      UNLIMITED_TAGS_MAP[tagName] ||
      hasAttributes(el, sd.para.heatmap.track_attr)
    );
  },
    ...
    var delayTime = interDelay({
      ...
      func: function (para, isClose) {
        ...
        if (
          ...
          para.event_duration =
            para.event_duration < 0 ? 0 : para.event_duration;
  ...
  autoTrackSinglePage: function (para, callback) {
    ...
    if (sd.is_first_visitor && is_set_profile) {
      ...
      if (
        ...
        eqidObj['$search_keyword_id_hash'] = hashCode53(
  ...
  if (
    ...
    result.$latest_referrer_host =
      result.$latest_referrer === ''
        ? ''
        : getHostname(result.$latest_referrer);
  }
  return result;
}
...
function iOSWebClickPolyfill() {
  ...
  if (isIOS() && getIOSVersion() && getIOSVersion() < 13) {
    ...
    if (sd.para.heatmap && sd.para.heatmap.track_attr) {
      setCssStyle(
        '[' + sd.para.heatmap.track_attr.join('], [') + ']' + default_cursor_css
      );
    }
...
kit.encodeTrackData = function (data) {
  ...
  return (
    'data=' + encodeURIComponent(dataStr) + '&ext=' + encodeURIComponent(crc)
  );
};
...
BatchSend.prototype = {
  ...
  getServerUrl: function () {
    if (
      ...
      this.serverUrl = isArray(sd.para.server_url)
  ...
  send: function () {
    if (
      this.sendTimeStamp &&
      now() - this.sendTimeStamp < sd.para.batch_send.datasend_timeout
    )
      return;
  ...
  request: function (data, dataKeys) {
    ...
    ajax({
      ...
      type: 'POST',
      data:
        'data_list=' + encodeURIComponent(base64Encode(JSON.stringify(data))),
      credentials: false,
  ...
  remove: function (dataKeys) {
    ...
    if (tabStorage) {
      var tabStorageData = (
        safeJSONParse(tabStorage) || this.generateTabStorageVal()
      ).data;
      ...
      _localStorage.set(
        this.tabKey,
        JSON.stringify(this.generateTabStorageVal(tabStorageData))
      );
    }
  },
  ...
  recycle: function () {
    ...
    for (var i = 0; i < localStorage.length; i++) {
      ...
      if (item.indexOf(tabStoragePrefix) === 0) {
        var tabStorage =
          safeJSONParse(_localStorage.get(item)) ||
          this.generateTabStorageVal();
        for (var j = 0; j < tabStorage.data.length; j++) {
          notSendMap[tabStorage.data[j]] = true;
        }
        if (
          now() > tabStorage.expireTime &&
          this.serverUrl === tabStorage.serverUrl
        ) {
          ...
          concurrentStorage.get(item, lockTimeout, 1000, function (data) {
            if (data) {
              ...
              var recycleData =
                safeJSONParse(data) || self.generateTabStorageVal();
              _localStorage.set(
                self.tabKey,
                JSON.stringify(
  ...
  window.sensorsdata_app_call_js.modules[this.type] = function (data) {
    try {
      ...
      if (messageId && that.resultCbs[messageId]) {
        data = tmp;
        if (
          that.timeoutCbs[messageId] &&
          that.timeoutCbs[messageId].isTimeout
        ) {
...
SDKJSBridge.prototype.call = function (callArg, timeout) {
  ...
  return {
    onResult: function (callback) {
      ...
      !that.timeoutCbs[callId].isTimeout &&
        that.resultCbs[callId].callbacks.push(callback);
      return this;
    },
    onTimeout: function (callback) {
      ...
      !that.resultCbs[callId].result &&
        that.timeoutCbs[callId].callbacks.push(callback);
      return this;
    },
  };
};
...
vtrackBase.configIsMatchNew = function (properties, eventConf) {
  if (
    isString(properties.$element_selector) &&
    isString(eventConf.element_selector)
  ) {
    if (
      eventConf.element_field === 'element_selector' &&
      eventConf['function'] === 'equal'
    ) {
      return properties.$element_selector === eventConf.element_selector;
    }
    if (
      eventConf.element_field === 'element_selector' &&
      eventConf['function'] === 'contain'
    ) {
      return (
        properties.$element_selector.indexOf(eventConf.element_selector) > -1
      );
    }
  }
  if (isString(properties.$element_path) && isString(eventConf.element_path)) {
    if (
      eventConf.element_field === 'element_path' &&
      eventConf['function'] === 'equal'
    ) {
...
vtrackBase.getProp = function (propConf, data) {
  ...
  if (propConf.method === 'content') {
    var el;
    if (
      isString(propConf.element_selector) &&
...
vtrackBase.addCustomProps = function (data) {
  if (sd.bridge.bridge_info.verify_success === 'success') {
    var h5_props = sd.vapph5collect.customProp.geth5Props(
...
var vapph5collect = {
  ...
  initAppUpdateConfigBridge: function () {
    var _this = this;
    return new sd.SDKJSBridge('updateH5VisualConfig').onAppNotify(function (
      data
    ) {
      if (data) {
        try {
...
function registerFeature(feature) {
  feature &&
    feature.dataStage &&
    dataStage.registerStageImplementation(feature.dataStage);
  feature &&
    feature.businessStage &&
    businessStage.registerStageImplementation(feature.businessStage);
  feature &&
    feature.sendStage &&
    sendStage.registerStageImplementation(feature.sendStage);
}
    ...
    each(o, function (a, b) {
      if (isObject(a)) {
        ...
        if (isString(a)) {
          ...
          o[b] = formatString(
            a,
            indexOf(white_list, b) > -1 ? 1024 : sdPara.max_string_length
          );
        }
      }
    });
...
var is_compliance_enabled = isObject(preCfg)
  ? preCfg.is_compliance_enabled
  : false;
...
try {
  ...
  sd.modules['AndroidBridge'] = (function () {
    ...
    function hoverNode(request) {
      return (
        vmode.call(vbridge) &&
        vhover &&
        vhover.call(vbridge, JSON.stringify(request))
      );
    }
    ...
    function callBridge(request, bridge) {
      return (
        bridge &&
        typeof bridge[request.callType] === 'function' &&
        bridge[request.callType]()
      );
    }
    ...
    function initBridge() {
      ...
      if (sd.para.app_js_bridge && !sd.para.app_js_bridge.is_mui) {
        sd.bridge.is_verify_success =
          anServerUrl && sd.bridge.validateAppUrl(anServerUrl);
      }
  ...
  sd.modules['AndroidObsoleteBridge'] = (function () {
    ...
    function initBridge() {
      ...
      if (
        !sd ||
        sd.bridge.activeBridge ||
        !(anVerify || anTrack || anVisualVerify)
      ) {
        return;
      }
  ...
  sd.modules['IosBridge'] = (function () {
    ...
    function initBridge() {
      ...
      if (sd.para.app_js_bridge && !sd.para.app_js_bridge.is_mui) {
        sd.bridge.is_verify_success =
          iosServerUrl && sd.bridge.validateAppUrl(iosServerUrl);
      }
  ...
  sd.modules['Utm'] = (function () {
      ...
      init: function (sa) {
        ...
        function getUtm() {
          var campaign_keywords = source_channel_standard.split(' '),
            ...
            params = {};
          if (
            sd._.isArray(sd.para.source_channel) &&
    ...
    if (
      ...
      window.SensorsDataWebJSSDKPlugin.Utm =
        window.SensorsDataWebJSSDKPlugin.Utm || utm;
    } else {
      window.SensorsDataWebJSSDKPlugin = {
        Utm: utm,
      };
