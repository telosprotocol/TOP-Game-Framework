...
function loadScript(args) {
  ...
  const features = args.fills
    ? `features=${args.neededPolyfills.join(',')}`
    : '';
  ...
  js.src = `https://cdn.polyfill.io/v2/polyfill${min}.js?${
    features + flags + monitor + agent + fallback
  }`;
  js.async = true;
  ...
  js.onerror = () => {
    throw new Error(
      'Error loading polyfills. Open a ticket: https://github.com/PascalAOMS/dynamic-polyfill/issues'
    );
  };
}
